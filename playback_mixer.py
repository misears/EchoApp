...
def _render_track_segment(project: Project, track_index: int, total_frames: int) -> np.ndarray:
...
def mix_project_to_segment(project: Project) -> np.ndarray:
    ...
    for track_index, track in enumerate(project.tracks):
        ...
        track_audio = _render_track_segment(project, track_index, master.shape[0])
        settings = track.playback_settings
        ...
        gain = float(10.0 ** (float(track.volume_db) / 20.0))
        master += track_audio * gain
